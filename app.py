...
def index():
    if 'user_id' in session:
        # After login, send user to selection page to choose Text or Audio input
        return redirect(url_for('select_input_page'))
    return redirect(url_for('login'))
...
def analyze_text():
    ...
    text = data.get('text', '')
    original_urdu = data.get('original_urdu', '')  # Get original Urdu text if available
    ...
    save_analysis(
        session['user_id'], 
        original_urdu,  # Store the original Urdu text 
        text, 
        prediction_result['prediction'],
        prediction_result['suicidal_prob'],
        prediction_result['non_suicidal_prob']
...
def admin_panel():
    ...
    # Get users with proper datetime conversion
    ...
    for row in cursor.fetchall():
        ...
        try:
            user['created_at'] = datetime.strptime(
                user['created_at'], 
                '%Y-%m-%d %H:%M:%S'  # SQLite timestamp format
            )
        except ValueError as e:
            print(f"Error parsing date: {user['created_at']} - {str(e)}")
            user['created_at'] = None  # Handle invalid dates
        users.append(user)
    ...
    # Get recent analyses with parsed dates
    cursor.execute("""
        SELECT 
            a.id,
            a.prediction,
    ...
    for row in cursor.fetchall():
        analysis = dict(row)
        analysis['created_at'] = datetime.strptime(analysis['created_at'], '%Y-%m-%d %H:%M:%S')
...
def delete_user(user_id):
    ...
    conn.commit()
    conn.close()
    flash(f'User ID {user_id} deleted successfully')
    return redirect(url_for('admin_panel'))


@app.route('/user_history/delete/<int:analysis_id>', methods=['POST'])
@login_required
def delete_analysis(analysis_id):
    # Ensure the analysis belongs to the logged-in user (or admin)
    conn = sqlite3.connect('suicide_app.db')
    cursor = conn.cursor()
    cursor.execute("SELECT user_id FROM analysis_history WHERE id = ?", (analysis_id,))
    row = cursor.fetchone()
    if not row:
        conn.close()
    ...
    if session.get('user_id') != owner_id and session.get('role') != 'admin':
        conn.close()
...
def user_history():
    ...
    # Get user's analysis history with proper column selection
    cursor.execute("""
        SELECT 
            id,
            original_text,
    ...
    for row in cursor.fetchall():
        ...
        analysis['created_at'] = datetime.strptime(
            analysis['created_at'], 
            '%Y-%m-%d %H:%M:%S'
        )
        analyses.append(analysis)
    ...
    print(f"Found {len(analyses)} analyses for user {session['user_id']}:")
    for analysis in analyses:
        print(f"- ID: {analysis['id']}, Text: {analysis['translated_text'][:50]}...")
    
    return render_template('user_history.html', analyses=analyses)


# New pages for separate inputs
@app.route('/select_input')
@login_required
def select_input_page():
